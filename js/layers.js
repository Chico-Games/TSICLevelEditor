...
class WorldLayer {
    ...
    /**
     * Export to base64-RLE format (ultra-compact) - async
     * Returns RLE data encoded as base64 string
     */
    async exportRLEDataBase64() {
        const totalTiles = this.width * this.height;
        const defaultColor = '#000000'; // Default empty color
        ...
        if (typeof base64RLEEncoder !== 'undefined') {
            return await base64RLEEncoder.encodeLayer(rle, palette, this.layerType, this.width, this.height);
        } else {
...
class LayerManager {
     ...
     */
    async fillLayerWithDefault(layer, defaultColor) {
        if (!layer || !defaultColor) return;

        const chunkSize = 32; // Process 32 rows at a time
        for (let startY = 0; startY < layer.height; startY += chunkSize) {
            // Yield to browser every chunk
            if (startY > 0) {
                await new Promise(resolve => setTimeout(resolve, 0));
            }

            const endY = Math.min(startY + chunkSize, layer.height);
     ...
     * Export to base64-RLE format (ultra-compact) - async
     */
    async exportRLEDataBase64(mapName = 'TSIC_Mall', description = 'Generated by TSIC Level Editor', seed = null) {
        const metadata = {
            name: mapName,
            description: description,
            world_size: this.width,
        ...
        const layers = await Promise.all(this.layers.map(layer => layer.exportRLEDataBase64()));
